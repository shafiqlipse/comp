...
from .models import Basketball3, B3Group, B3Fixture


@receiver(post_save, sender=Basketball3)
def create_groups_for_season(sender, instance, created, **kwargs):
    if created:
        ...
        for i in range(1, num_groups + 1):
            ...
            B3Group.objects.create(name=group_name, competition=instance)
...
@receiver(pre_save, sender=B3Fixture)
def reset_scores_if_inplay(sender, instance, **kwargs):
    if instance.id:
        previous = B3Fixture.objects.get(id=instance.id)
