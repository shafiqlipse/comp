...
def get_rankings(competition):
    # Athlete Rankings
    athlete_rankings = (
        Athlete.objects.filter(b5athlete__match__competition=competition)
        .annotate(
            points=Sum(
                Case(
                    When(b5athlete__event_type="1-Pointer", then=1),
                    When(b5athlete__event_type="2-Pointer", then=2),
                    When(b5athlete__event_type="3-Pointer", then=3),
                    default=0,
                    output_field=IntegerField(),
                )
            ),
            two_pointers=Count(
                "b5athlete", filter=Q(b5athlete__event_type="2-Pointer")
            ),
            three_pointers=Count(
                "b5athlete", filter=Q(b5athlete__event_type="3-Pointer")
            ),
            rebounds=Count("b5athlete", filter=Q(b5athlete__event_type="rebound")),
            assists=Count("b5athlete", filter=Q(b5athlete__event_type="Assist")),
            steals=Count("b5athlete", filter=Q(b5athlete__event_type="steal")),
            blocks=Count("b5athlete", filter=Q(b5athlete__event_type="Block")),
            fouls=Count("b5athlete", filter=Q(b5athlete__event_type="Foul")),
        )
    ...
    # Team Rankings
    team_rankings = (
        SchoolTeam.objects.filter(basket5_team__match__competition=competition)
        .annotate(
            points=Sum(
                Case(
...
def B5bol(request, id):
    competition = Basketball5.objects.get(id=id)
    b5groups = B5Group.objects.filter(competition=competition)
    pending_fixtures = B5Fixture.objects.filter(
        status="Pending", competition=competition
    ).order_by("date")
    results = B5Fixture.objects.filter(
        status="InPlay", competition=competition
    ).order_by("date")
    rankings = get_rankings(competition)

    standings_data = {}
    groups = B5Group.objects.filter(competition=competition)
    ...
    for group in groups:
        ...
        group_fixtures = B5Fixture.objects.filter(group=group)
    ...
    return render(request, "frontend/basketball5.html", context)


from django.db.models import Q


def FB5FixtureDetail(request, id):
    ...
    # ------fixtures by team
    team1_fixtures = B5Fixture.objects.filter(
        Q(team1=fixture.team1) | Q(team2=fixture.team1)
    ).distinct()
    team2_fixtures = B5Fixture.objects.filter(
...
def B5home(request):
    ...
    context = {}
    return render(request, "frontend/b5home.html", context)


def b5bollStandings(request):
    sports = Sport.objects.all()

    standings_data = []
    for sport in sports:
        basketball5_competitions = Basketball5.objects.filter(sport=sport)

        for basketball5 in basketball5_competitions:
...
def competition_rankings(request, competition_id):
    competition = get_object_or_404(Basketball5, id=competition_id)
    rankings = get_rankings(competition)
