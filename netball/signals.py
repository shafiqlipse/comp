...
from .models import Netball, NGroup


# create groups
@receiver(post_save, sender=Netball)
def create_groups_for_season(sender, instance, created, **kwargs):
    if created:
        ...
        for i in range(1, num_groups + 1):
            ...
            NGroup.objects.create(name=group_name, competition=instance)


# set score to zero and begin th game
@receiver(pre_save, sender=NFixture)
def reset_scores_if_inplay(sender, instance, **kwargs):
    if instance.id:
        previous = NFixture.objects.get(id=instance.id)
