...
def Furtbol(request, id):
    competition = Rugby7.objects.get(id=id)
    fgroups = R7Group.objects.filter(competition=competition)
    ...
    return render(request, "frontend/rugby7.html", context)
...
def competition_rankings(request, competition_id):
    competition = get_object_or_404(Rugby7, id=competition_id)
    rankings = get_rankings(competition)
    ...
    return render(request, "frotend/rug7rankings.html", context)
