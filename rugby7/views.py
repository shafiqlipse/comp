...
def Rug7(request):
    rugby7 = Sport.objects.get(name="Rugby7")
    fcomps = Rugby7.objects.filter(sport=rugby7)
    ...
    if request.method == "POST":
        ...
        if cform.is_valid():
            ...
            competn.save()
            return HttpResponseRedirect(reverse("rugby7"))
    else:
        cform = CompForm()

    # If form is invalid, re-render form with errors
    context = {"cform": cform, "fcomps": fcomps}
    return render(request, "server/rugby7.html", context)


# delete
def delete_rugby7(request, id):
    rugby7 = get_object_or_404(Rugby7, id=id)

    if request.method == "POST":
        rugby7.delete()
        return redirect("comps")  # Redirect to the official list page or another URL

    return render(request, "comps/delete_comp.html", {"rugby7": rugby7})
...
from django.forms import inlineformset_factory
from django.http import JsonResponse
from .models import Rugby7, FGroup, Fixture
from .forms import FGroupForm, FixtureForm


def ftourn_details(request, id):
    tournament = get_object_or_404(Rugby7, id=id)
...
def generate_rug7fixtures_view(request, id):
    rugby7 = get_object_or_404(Rugby7, id=id)
    season = rugby7.season
    now = datetime.now()

    time = now.time()
    # Fetch all teams for the rugby7 (assuming you have a Team model)
    teams = SchoolTeam.objects.all()

    # Fetch all groups for the rugby7
    groups = FGroup.objects.filter(competition=rugby7)
...
def rugby7Standings(request):
    sports = Sport.objects.all()

    standings_data = []
    for sport in sports:
        rugby7_competitions = Rugby7.objects.filter(sport=sport)

        for rugby7 in rugby7_competitions:
            groups = FGroup.objects.filter(competition=rugby7)
...
def generate_knockout_rug7fixtures(request, id):
    rugby7 = get_object_or_404(Rugby7, id=id)
    season = rugby7.season
    now = datetime.now()
    ...
    for group in groups:
        group_fixtures = Fixture.objects.filter(
            competition=rugby7, group=group, stage="Group"
        )
    ...
    if num_teams >= 2:
        ...
        if fixtures:
            ...
            print(f"Not enough teams to generate fixtures for {rugby7}")
    else:
        print(f"Not enough teams to generate fixtures for {rugby7}")
