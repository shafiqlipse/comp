...
def R7home(request):
    ...
    context = {}
    return render(request, "frontend/r7home.html", context)
...
def get_rankings(competition):
    # Player Rankings
    player_rankings = (
        Athlete.objects.filter(r7player__match__competition=competition)
        .annotate(
            tries=Count("r7player", filter=Q(r7player__event_type="Try")),
            conversions=Count("r7player", filter=Q(r7player__event_type="Conversion")),
            penalty_kicks=Count(
                "r7player", filter=Q(r7player__event_type="PenaltyKick")
            ),
            drop_goals=Count("r7player", filter=Q(r7player__event_type="DropGoal")),
            tackles=Count("r7player", filter=Q(r7player__event_type="Tackle")),
            yellow_cards=Count("r7player", filter=Q(r7player__event_type="YellowCard")),
        )
    ...
    # Team Rankings
    team_rankings = (
        SchoolTeam.objects.filter(r7team__match__competition=competition)
        .annotate(
            tries=Count("r7team", filter=Q(r7team__event_type="Try")),
            conversions=Count("r7team", filter=Q(r7team__event_type="Conversion")),
            penalty_kicks=Count("r7team", filter=Q(r7team__event_type="PenaltyKick")),
            drop_goals=Count("r7team", filter=Q(r7team__event_type="DropGoal")),
            tackles=Count("r7team", filter=Q(r7team__event_type="Tackle")),
            yellow_cards=Count("r7team", filter=Q(r7team__event_type="YellowCard")),
        )
        .order_by("-tries")
    )
...
def Rug7s(request, id):
    competition = Rugby7s.objects.get(id=id)
    r7groups = R7Group.objects.filter(competition=competition)
    pending_fixtures = R7Fixture.objects.filter(
        competition=competition, status="Pending"
    ).order_by(F("group").asc(nulls_last=True), "date", "time")
    Results = R7Fixture.objects.filter(
        status="InPlay", competition=competition
    ).order_by(F("group").asc(nulls_last=True), "date", "time")
    rankings = get_rankings(competition)

    standings_data = {}
    groups = R7Group.objects.filter(competition=competition)
    ...
    for group in groups:
        ...
        group_fixtures = R7Fixture.objects.filter(group=group)
    ...
    return render(request, "frontend/rugby7s.html", context)


def R7Groups(request):
    ngroups = R7Group.objects.all()
...
def FixtureDetail(request, id):
    fixture = get_object_or_404(R7Fixture, id=id)
    ...
    # ------fixtures by team
    team1_fixtures = R7Fixture.objects.filter(
        Q(team1=fixture.team1) | Q(team2=fixture.team1)
    ).distinct()
    team2_fixtures = R7Fixture.objects.filter(
    ...
    return render(request, "frontend/r7fixture.html", context)


def R7Fixtures(request):
    fixtures = R7Fixture.objects.filter(status="InPlay").order_by("-date")
    context = {"fixtures": fixtures}
    return render(request, "frontend/rug7fixtures.html", context)


def R7Results(request):
    results = R7Fixture.objects.filter(status="InPlay").order_by("-date")
    context = {"results": results}
    return render(request, "frontend/rug7fixtures.html", context)


def rug7bollStandings(request):
    sports = Sport.objects.all()

    standings_data = []
    for sport in sports:
        rug7ball_competitions = Rugby7s.objects.filter(sport=sport)

        for rug7ball in rug7ball_competitions:
            groups = R7Group.objects.filter(competition=rug7ball)
            ...
            for group in groups:
                ...
                fixtures = R7Fixture.objects.filter(group=group)
    ...
    return render(request, "frontend/rug7standings.html", context)
