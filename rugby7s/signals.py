...
from .models import Rugby7s, R7Group


# create groups
@receiver(post_save, sender=Rugby7s)
def create_groups_for_season(sender, instance, created, **kwargs):
    if created:
        ...
        for i in range(1, num_groups + 1):
            ...
            R7Group.objects.create(name=group_name, competition=instance)


# set score to zero and begin th game
@receiver(pre_save, sender=R7Fixture)
def reset_scores_if_inplay(sender, instance, **kwargs):
    if instance.id:
        previous = R7Fixture.objects.get(id=instance.id)
