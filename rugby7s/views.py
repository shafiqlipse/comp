...
@school_required
def Ragby7(request):
    rugby7s = Sport.objects.get(name="Rugby 7s")
    fcomps = Rugby7s.objects.filter(sport=rugby7s)
    ...
    if request.method == "POST":
        ...
        if cform.is_valid():
            ...
            competn.save()
            return HttpResponseRedirect(reverse("rugby7s"))
    else:
        cform = CompForm()

    # If form is invalid, re-render form with errors
    context = {"cform": cform, "fcomps": fcomps}
    return render(request, "server/rugby7s.html", context)


# Create your views here.
@school_required
def delete_rugby7s(request, id):
    rugby7s = get_object_or_404(Rugby7s, id=id)

    if request.method == "POST":
        rugby7s.delete()
        return redirect("comps")  # Redirect to the official list page or another URL

    return render(request, "comps/delete_comp.html", {"rugby7s": rugby7s})
...
# Create your views here.
@school_required
def r7tourn_details(request, id):
    tournament = get_object_or_404(Rugby7s, id=id)
    r7groups = R7Group.objects.filter(competition=tournament)
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            formset.save()
            return redirect("rugby7s_tournament", tournament.id)

        fixture_form = R7FixtureForm(request.POST)
        ...
        fixture_form = R7FixtureForm()

    fixtures = R7Fixture.objects.filter(competition=tournament)
    results = R7Fixture.objects.filter(competition=tournament, status="InPlay")
    ...
    return render(request, "server/r7tournament.html", context)
...
# Create your views here.
@school_required
def generate_r7fixtures_view(request, id):
    rugby7s = get_object_or_404(Rugby7s, id=id)
    season = rugby7s.season
    now = datetime.now()
    time = now.time()
    # Fetch all teams for the rugby7s (assuming you have a Team model)
    teams = SchoolTeam.objects.all()

    # Fetch all groups for the rugby7s
    groups = R7Group.objects.filter(competition=rugby7s)

    # Implement your fixture generation logic here
    fixtures = []
    for group in groups:
        group_teams = teams.filter(r7group=group)
    ...
    R7Fixture.objects.bulk_create(fixtures)

    return JsonResponse({"success": True, "message": "Fixtures generated successfully"})


def edit_r7fixtures_view(request, id):
    fixture = get_object_or_404(R7Fixture, id=id)

    if request.method == "POST":
        form = R7FixtureForm(request.POST, instance=fixture)
        if form.is_valid():
            form.save()
            return redirect(
                "r7fixture", id=id
        ...
        form = R7FixtureForm(instance=fixture)

    return render(
        request, "server/r7edit_fixture.html", {"form": form, "fixture": fixture}
    )
...
def R7FixtureDetail(request, id):
    fixture = get_object_or_404(R7Fixture, id=id)
    ...
    if request.method == "POST":
        if "official_form" in request.POST:
            ...
            if cform.is_valid():
                ...
                new_official.save()
                return redirect("r7fixture", id=id)
    ...
    return render(request, "server/r7fixture.html", context)


# Create your views here.
@school_required
def fixtures(request):
    fixtures = R7Fixture.objects.filter(competition_id=4).order_by("-date")
    context = {"fixtures": fixtures}
    return render(request, "server/r7fixtures.html", context)


def rugby7sStandings(request):
    sports = Sport.objects.all()

    standings_data = []
    for sport in sports:
        rugby7s_competitions = Rugby7s.objects.filter(sport=sport)

        for rugby7s in rugby7s_competitions:
            groups = R7Group.objects.filter(competition=rugby7s)
...
from .models import Sport, Rugby7s, R7Group, R7Fixture
...
def create_r7fixture(request):
    if request.method == "POST":
        ...
        if fixture_form.is_valid():
            ...
            rugby7s_competition = Rugby7s.objects.get(id=1)
            fixture.competition = rugby7s_competition
